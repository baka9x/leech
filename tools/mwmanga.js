...
function getOriginalImageFromSrcSet(srcSet,dirName){
    ...
    const pattern2 = /https:\/\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?).png/g
    const pattern3 = /https:\/\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?).jpeg/g
   
    const patt = /\?resize=(.*?)\&ssl\=1/g
    //https:\/\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?).png
   
    imgSrc = srcSet.replace(pattern, '<p><img src="https://baka1.mwmanga.net/series/' +dirName+ '/$7.jpg"/></p>')
    imgSrc = imgSrc.replace(pattern2, '<p><img src="https://baka1.mwmanga.net/series/' +dirName+ '/$7.png"/></p>')
    imgSrc = imgSrc.replace(pattern3, '<p><img src="https://baka1.mwmanga.net/series/' +dirName+ '/$7.jpeg"/></p>')
    imgSrc = imgSrc.replace(patt, '')
    imgSrc = imgSrc.replace('?ssl=1','')
    imgSrc = imgSrc.replace('/15/','/')
    ...
    // const pattern4 = /https:\/\/(.*?)\.jpeg/g
    // const ads3 = /\?resize\=(.*?)\&ssl=1/g
    ...
    // imgSrc = imgSrc.replace(pattern4, '<p><img src="https://$1.jpeg"/></p>')
    // imgSrc = imgSrc.replace(ads3,'')
...
async function getImagesFromUrl(url){
    ...
    const imageSrcSets = await page.evaluate(() =>{
        const imgs = Array.from(document.querySelectorAll('.chapter_beta_content img'))
...
async function main(){
  ...
  if(!fs.existsSync(resultFolder)){
    ...
    let urlLink = readline.question("Nhap Link mwmanga: ")
    ...
    images.forEach((img) => {
      ...
      const rm = /https:\/\/i(.*?)\.wp.com\/(.*?)\?ssl=1/g
      img = img.replace(rm,'https://$2')
    
      downloader.image({
          Referer: 'https://doctruyen3q.net',
          url: img,
          dest: resultFolder
      })
  })
